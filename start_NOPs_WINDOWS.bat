...
for /d %%d in ("!HOUDINI_INSTALL_ROOT!\Houdini 22.*") do (
    REM Get the version number from the last *.num in directory name
    set "version=%%~nxd"
    @REM echo !version!
    REM Remove the "Houdini 22.0." prefix
    set "version=!version:Houdini 22.0.=!"
    REM Compare the version number with the current highest version
    if !version! gtr !highestVersion! (
        set "highestVersion=!version!"
    )
)
REM if highestVersion is still 0, then no Houdini 22.0.* directory was found
if !highestVersion! equ 0 (
    echo No C:\Program Files\Side Effects Software\Houdini 22.0* directories found
    echo Either install h22 or change first line in start_NOPs.bat to the correct the install *ROOT*
    pause
    exit /b
)
echo Launching highest version: 22.0.!highestVersion!
REM Start Houdini with the highest version
start "" "C:\Program Files\Side Effects Software\Houdini 22.0.!highestVersion!\bin\houdini.exe"
